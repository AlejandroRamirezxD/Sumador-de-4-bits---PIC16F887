# 1 "MainEntregableDelLab1-ARamirez.s"
# 1 "<built-in>" 1
# 1 "MainEntregableDelLab1-ARamirez.s" 2
...
# 12 "MainEntregableDelLab1-ARamirez.s" 2
 ...
 loop:
    btfsc PORTB, 0
    call inc_porta ; Cuando ((PORTB) and 07Fh), 0 sea 1, incrementar el puerto A

    btfsc PORTB, 1
    call dec_porta ; Cuando ((PORTB) and 07Fh), 1 sea 1, decrecer el puerto A

    btfsc PORTB, 2
    call inc_portc ; Cuando ((PORTB) and 07Fh), 2 sea 1, incrementar el puerto C

    btfsc PORTB, 3
    call dec_portc ; Cuando ((PORTB) and 07Fh), 3 sea 1, decrecer el puerto C


    btfss PORTB, 4
    call sumaAyC ; Cuando ((PORTB) and 07Fh), 4 sea 0, sumar los valores de A y C

    goto loop

 ;-----------SUB RUTINAS--------------
 inc_porta:
    ; Antirrebote de ((PORTB) and 07Fh), 0
    call delay_small
    btfsc PORTB, 0
    goto $-1

    ; Si es 0 el ((PORTB) and 07Fh), 0 incrementar PORTA
    incf PORTA
 ...
 dec_porta:
    ; Antirrebote de ((PORTB) and 07Fh), 1
    call delay_small
    btfsc PORTB, 1
    goto $-1

    ; Si es 0 el ((PORTB) and 07Fh), 1 decrecer PORTA
    decf PORTA
  ...
  inc_portc:
    ; Antirrebote de ((PORTB) and 07Fh), 2
    call delay_small
    btfsc PORTB, 2
    goto $-1

    ; Si es 0 el ((PORTB) and 07Fh), 2 incrementar PORTC
    incf PORTC
  ...
  dec_portc:
    ; Antirrebote de ((PORTB) and 07Fh), 3
    call delay_small
    btfsc PORTB, 3
    goto $-1

    ; Si es 0 el ((PORTB) and 07Fh), 3 decrecer PORTC
    decf PORTC
...
config_reloj:
    banksel OSCCON
    bsf ((OSCCON) and 07Fh), 6 ; OSCCON, bit 6
    bcf ((OSCCON) and 07Fh), 5 ; OSCCON, bit 5
    bcf ((OSCCON) and 07Fh), 4 ; OSCCON, bit 4
    bsf ((OSCCON) and 07Fh), 0 ; OSCCON, bit 0
    return
